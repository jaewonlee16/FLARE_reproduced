...
from .utils import Utils, NoKeyAvailable, openai_api_call, phi_api_call
...
class QueryAgent:
    def __init__(
        ...
        if self.prefix_method in {'sentence', 'all'} or (self.prefix_method and self.prefix_method.startswith('freq:')):  # no truncation when computing PPL
            ...
            self.max_generation_len = 100  # no limit
    ...
    def prompt(
        ...
        if self.use_retrieval:
            return self.ret_prompt_self_check(queries, api_key=api_key)
    ...
    def ret_prompt_self_check(
        ...
        while len(queries) and max_gen_len < self.max_generation_len and step_ind <= max_iteration:
            ...
            if self.use_gold:
                ...
                # TODO=================================
                few_shot_prompt = """Search results ranked based on relevance in descending order:
...
Answer: Kurram Garhi is located in Pakistan. Trojkrsti is not found in the search results. However, based on the search results, it appears that Trojkrsti is not a known location. If you can provide more information or context, I can try to help you further.
Keywords for Further Search: Kurram Garhi location, Trojkrsti location, Trojkrsti country\n\n"""
                self_check_prompt = apireturns[0].prompt + apireturns[0].text
                responses = phi_api_call(
                    model=self.model,
                    prompt=[few_shot_prompt + self_check_prompt + "\nKeywords for Further Search: "],
                    max_tokens=64
                    )
                keywords = responses['choices'][0]['text']
                # Extracting the string before "Search results"
                search_prefix = "\nSearch"
                queries_to_issue = [keywords.split(search_prefix)[0].strip()]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model', type=str, default='text-davinci-003', choices=['gpt-3.5-turbo-instruct', "microsoft/Phi-3.5-mini-instruct", "meta-llama/Llama-3.2-3B-Instruct"])
    ...
    prompt_tokenizer = AutoTokenizer.from_pretrained('meta-llama/Llama-3.2-3B-Instruct')
