...
def escape_double_quotes(text):
    # Replace each double quote with an escaped version
    escaped_text = text.replace('"', '\\"')
    return escaped_text

def phi_api_call(*args, **kwargs):
    model_name = kwargs['model']
    prompt = kwargs['prompt']
    max_tokens = kwargs['max_tokens']
    prompt = escape_double_quotes(prompt[0])
    result = None
    # print(prompt)
    while True:
        ssh_error = False
        try:
            ...
            command = f'python llm_server.py """{prompt}""" {max_tokens}'
            #command = "conda env list"
            client = paramiko.SSHClient()
            client.set_missing_host_key_policy(paramiko.AutoAddPolicy())

            client.connect(hostname='147.46.92.217', username='root',password='abcd', port=31117, timeout=30)
            ...
            with sftp.open("/root/output.json") as file:
